...
```{r, eval=FALSE}
# load this sbml-file:
# https://raw.githubusercontent.com/jpahle/DynamicPublicationMediaManuscript/main/models/BIOMD0000000329_url.xml
...
```{r}
scan_values_df <- tibble(Galpha = rnorm(10, mean = species[1], sd = sd[1]),
...
for (scan in 1:nrow(scan_values_df)) {
  setSpecies(key = species(), initial_concentration = unlist(scan_values_df[scan, ]))
  
  tc_new <- runTimeCourse(duration = 24, intervals = 100)$result
  tc_list[scan] <- list(tc_new)
}
