...
Next, we will set everything up to run 1000 Timecouses in parallel.
...
`timeseries` is a list of 1000 tibbles with each tibble being time course. 
...
plotdata <-
  ...
  summarise(across(everything(), list(mean = stats::mean, sd = stats::sd)), .groups = "drop") %>%
...
Very similarly we will now scan over random concentrations of two species.
This implements an example from the [Mendes 2009 paper](https://doi.org/10.1007/978-1-59745-525-1_2) [^2] on COPASI use cases. 
...
# loadSBML("https://raw.githubusercontent.com/jpahle/DynamicPublicationMediaManuscript/main/models/BIOMD0000000068_url.xml")
...
This time, we will run 10000 stady-state calculations with random concentrations of two species (cysteine and adomed). 
