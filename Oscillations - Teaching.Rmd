...
newModel()
newSpecies("prey", initial_concentration = 100)
newSpecies("predator", initial_concentration = 2)

newReaction("predator + 5 * prey -> 2 * predator")
newReaction("prey + prey -> 3 * prey")
newReaction("predator ->")



renderPlot({
  #again, do I really need to call that?
  res <- runTC(duration = 1000)$result %>% 
...
```
This is a representation of a so called predator-prey model from Lotka-Volterra (DATE!)
...
First, we will only create these. The relationship between these species will be introduced in the rate laws later
...
We want to create a reaction for breeding and one for dying. For cats this might look like this:
```{r}
#Cats being born:
newReaction("Cats -> 2 * Cats")

#Cats dying:
newReaction("Cats ->")
```
For the Mice it looks similar. Try it!
...
Instead we will multiply mice and cats:
```{r}
newReaction("Cats -> 2 * Cats")
newReaction("Mice -> 2 * Mice")
```
Because we are dealing in concentrations (in our example that would be mice / square meter or similar) we can say that one mouse can create two mice, even if that doesn't necessarily make sense.
# {-}



# Rate laws {.tabset .tabset-pills}
We will now make the rate laws for our reactions.
The underlying equations are the following:

$$ dx/dt = ax - bxy$$
$$ dy/dt = dxy - gy $$
$x$ is the number of prey (Mice), $y$ is the number of some predators (Cats), $a,b,d,y$ are parameters for the interaciton of these species.

These differential equations show the growth rates for the populations, but for our rate laws, we need 
...
In a helpful review from Novák and Tyson (2008) we find the following requirements for oscillators:
