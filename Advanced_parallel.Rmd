...
This is another example from the [Condor-Copasi paper](https://doi.org/10.1186/1752-0509-6-91).
...
Now we will set everything up to run 1000 Timecouses in parallel. We need to export our model to the workers, as well as load CoRC and our model for each worker.
Afterwards, we seet 1000 random seeds so each time course will get a different random seed and last we actually call the timecourse function.
```{r}
timeseries <-
...
Very similarly we will now scan over random concentrations of two species. This implements an example from the [Mendes 2009 paper](https://link.springer.com/protocol/10.1007%2F978-1-59745-5) on COPASI use cases. 
