...
**The actors**
In a model, parts that can interact with each other are called *species*.
For the first model, we assume, that there are three species.
...
If a suseptible individual interacts with an infected individual, they are now **both** infected.
This process is called *infection*.
An infected individual can *recover*.
We can controll two parameters here.
