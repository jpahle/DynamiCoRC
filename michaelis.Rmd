...
Make sure they are installed before beginning.
...
Next, we can put in our reactions. Species that are called in the reaction creation and not yet part of the model, will be added automatically.
...
The runTimeCouse() command gives us a list with a lot of different aspects, but for now, only the "result" section is interesting to us.
...
The derivation of the Michaelis-Menten kinetics from this is similar to above, so we will skip that.

# Comparing models

Now, we want to show how our Michaelis-Menten-Kinetics work.
For this, we will plot the substrate and product concentration of our model next to the substrate and product concentration of a model with inbuilt Michaelis-Menten-Kinetics that looks like this:
...
As we can see, our inbuilt reaction still needs some parameters set. The $K_M$ we can easily be set with our formula which was derived earlier:
...
This is the basis for the Quasi-Steady-State and Equilibrium assumption.
...
We will now find the "Michaelis-Menten-Like" kinetics for the following reaction, where the reaction is reversible:
