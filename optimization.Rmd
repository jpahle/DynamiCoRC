...
Make sure you have these packages downloaded.
...
The model is stored in this repository but can also be loaded directly from the biomodels database (see commented out line).

```{r}
loadSBML("https://raw.githubusercontent.com/jpahle/DynamicPublicationMediaManuscript/main/models/BIOMD0000000023_url.xml")
...
We want to maximize our productivity, which we can define as the ratio of fluxes of accumulation over hydrolysis.
$J_{v11}/J_{v9} $.
For our optimization algorithms it is necessary to define our optimization goal as a minimum: we want to minimize $J_{v9}/J_{v11}$
...
We have to define our optimization parameters in **CoRC**.
...
Now we have to choose which optimization method we will use for optimization.
...
Now we have all elements we need for our optimization routine.
...
The dashed lines were from the timecourse before the optimization, so we can see that our optimization routine worked very well!
